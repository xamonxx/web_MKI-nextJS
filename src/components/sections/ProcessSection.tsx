        ...
        <div className="mt-12 grid grid-cols-2 gap-4 md:mt-16 md:gap-6 lg:grid-cols-5">
          {processSteps.map((item, index) => {
            ...
            <div 
              key={item.step} 
              className={cn(
                "group relative flex min-h-[140px] md:min-h-[180px] flex-col justify-between overflow-hidden rounded-3xl bg-card p-5 md:p-6 shadow-sm border border-border/50",
            ...
            >
              ...
              <div className="pointer-events-none absolute -bottom-4 -right-1 select-none text-[80px] font-black leading-none text-border transition-all duration-700 group-hover:scale-110 group-hover:text-mki-orange/5 md:text-[100px]">
                {item.step}
              </div>
              ...
              <div className="relative z-10 flex h-full flex-col">
                ...
                <div className="mb-4 flex items-center justify-between md:mb-6">
                  <div className="flex h-10 w-10 items-center justify-center rounded-2xl bg-secondary text-sm font-black text-foreground transition-all duration-500 group-hover:-rotate-3 group-hover:bg-mki-gradient group-hover:text-white group-hover:shadow-glow md:h-12 md:w-12 md:text-base">
                    {item.step}
                  </div>
                </div>
                
                {/* Title */}
                <h3 className="mt-auto text-sm font-extrabold leading-snug text-foreground transition-colors duration-300 group-hover:text-mki-orange md:text-base md:leading-relaxed">
                  {item.title}
                </h3>
              </div>
            </div>
